...
def build_row_key(cpf_digits: str) -> str:
...
def build_conferencia_cpf_valor(df_a: pd.DataFrame, df_b: pd.DataFrame) -> list[dict]:
    """
    TRIER (A) and MINERVA (B) both have CPF and Valor columns
    Output: one row per CPF (comparing totals)
    """
    ...
    # ---- MINERVA: group by CPF and sum values
    b_grp = (
    ...
    for _, row in a_grp.iterrows():
        cpf = row["cpf_norm"]
        trier_val = float(row["Valor_num"])
        trier_nome = str(row["cliente"])
        ...
        if minerva_info is not None:
            ...
            diff = abs(trier_val - minerva_total)
...
def read_existing_annotations(ws_out) -> dict:
    ...
    for row in values[1:]:  # Skip header
        row = row + [""] * (7 - len(row))  # Updated for 7 columns
...
def main():
    ...
    ensure_sheet_size(ws_out, min_rows=max(2000, len(items) + 5), min_cols=7)  # Updated to 7 columns
    ...
    for d in items:
        ...
        rows.append([
            format_cpf(cpf_digits),  # CPF
            trier_nome,              # TRIER
            trier_val_fmt,            # Valor (TRIER)
            minerva_nome,             # MINERVA
            minerva_val_fmt,          # Valor (MINERVA)
            status_final,             # STATUS
            anot                      # Anotações
        ])
    ...
    new_len = len(values)
    if prev_len > new_len:
        clear_leftover_rows(ws_out, start_row=new_len + 1, end_row=prev_len, end_col_letter="G")  # Updated to G

    # Remove any existing data validation from STATUS column
    try:
        # Clear data validation from column F (index 5) - STATUS column
